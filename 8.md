...
用户界面部分的详情将在[第十章 Odoo 18开发之后台视图 - 设计用户界面](10.md)中讨论，看板视图在[第十一章 Odoo 18开发之看板视图和用户端 QWeb](11.md)中讨论。我们来快速过一遍数据模型吧。
...
这类业务逻辑大多涉及读取和写入记录节。详细的技巧在[第七章 Odoo 18开发之记录集 - 使用模型数据](7.md)中进行了讨论，在其中我们提供了实现实际业务逻辑的工具。
...
模型定义逻辑在[第六章 Odoo 18开发之模型 - 结构化应用数据](6.md)中进行了讨论。相关的例子可见**数据模型模式**一节。**记录集的ORM方法装饰器**一节提供对一些这里所说的ORM装饰器的回顾。
...
在[第六章 Odoo 18开发之模型 - 结构化应用数据](6.md)中进行过详细的讨论。
...
可以在`  <field>  `内声明具体的视图。在以上代码就这么做的。在**line_ids**字段元素内，有一个内嵌的`  <list>  `视图定义在该表单中使用。
...
以上代码读者应该习以为常了。阶段有一个逻辑排序，所以呈现的顺序很生育。这借由 **_order="sequence"** 来实现。我们还看到**state**字段将每个阶段与一个基本状态相映射，在业务逻辑中可安全使用。
...
在按阶段分组时，会看到所有的阶段，而不仅仅是那些有借阅记录的。使用的方法返回分组使用的记录集。本例中，返回所有活跃阶段是恰当的：
...
导入、导出操作在[第五章 Odoo 18开发之导入、导出以及模块数据](5.md)已做讨论，也可以通过 ORM API 中的如下方法操作：
...
其一是它与服务端事件无关联。onchange仅在表单视图请求时起作用，但在实际**write()** 值变更时不会调用。这要求服务端业务逻辑重写相关的onchange方法。
...
同一应用还由**mail.activity.mixin**模型提供活动管理功能。可对表单视图添加活动邮件，让用户可以计划、追踪活动历史。

### 添加消息和活动功能

mail 模块提供包含**mail.thread**抽象类，用于对任意模型添加消息功能。还提供**mail.activity.mixin**用于添加规划活动功能。在[第四章 Odoo 18开发之模块继承](4.md)中已讲解了如何使用mixin 抽象类的继承来为模型添加这些继承功能。
...
-   **备注**：带有**mail.mt_note** XML ID，用于创建带有**Log note** XML ID的消息，默认不会发送通知。

应用可添加自己的子类型，通常与相关的活动关联。例如，**Sales**应用添加了两个子类型：**报价已发送**和**销售订单已确认**。在消息历史中记录这些事件时会由应用的业务逻辑所用。

子类型允许我们决定何时发送通知以及发送给谁。消息组件右上角的关注者菜单允许我们新增或删除关注者，以及选择接收通知的子类型。下图展示了某一关注者Gemini Furniture的子类型选择表单(点击关注者再编辑)：
